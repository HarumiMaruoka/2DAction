...
/// </summary>
public class NewDrawAlteration : UseEventSystemBehavior
{
    //===== フィールド / プロパティ =====//
    ...
    /// 変化するステータスの量を描画する。<br/>
    /// 更新のタイミング : <br/>
    /// 「選択している装備」が変わった時に実行する。<br/>
    /// </summary>
    void Update_AlterationValue(GameObject currentButton, GameObject beforeButton)
    {
        //選択対象が「装備」かどうか判定する。
        if (currentButton != null)
        {
            // 変化量を描画する
            ChangeAlterationValue(currentButton.TryGetComponent(out EquipmentButton equipment));
            // 選択中の装備の種類が腕の場合
            if (equipment?._myEquipment._myType == Equipment.EquipmentType.ARM_PARTS)
            {
                // 腕用のアニメーションを再生する。初期値は右腕。
                _armType = Constants.RIGHT_ARM;
                _animator.Play(_animName_EquipmentAlterationValue);
            }
            else
            {
                // そうでない場合は、空のステートを再生する。
                _animator.Play(_animName_None);
            }
        }
        else
        {
            // 選択されているものが"EquipmentButton"でなければ変化量を描画しない。
